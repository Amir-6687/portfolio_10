...
export function NeuralBackground() {
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      const newHeight = Math.max(document.documentElement.scrollHeight, document.documentElement.clientHeight, window.innerHeight)
    ...
    const animate = () => {
      ...
      const currentHeight = Math.max(document.documentElement.scrollHeight, document.documentElement.clientHeight, window.innerHeight)
  ...
  return (
    <canvas ref={canvasRef} className="pointer-events-none absolute top-0 left-0 z-0" style={{ mixBlendMode: blendMode, opacity: opacity, width: "100%" }} />
  )
}
